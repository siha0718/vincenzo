...
# Load Yolo
net = cv2.dnn.readNet("model/yolov2-tiny.weights", "model/yolov2-tiny.cfg")
...
while True:
        ...
        #count_limit = height // 2 #가로줄할때
        count_limit = width // 2 #세로줄
        
        
        # draw a horizontal line in the center of the frame
        #cv2.line(frame, (0, count_limit), (width, count_limit), (0, 255, 255), 2)  #가로줄
        cv2.line(frame, (count_limit, 0), (count_limit, width), (0, 255, 255), 2) #세로줄
        ...
        #수용인원 산정 방법 : 바닥면적 / 1.9m^2
        
        rush_ratio = ( person_count / (value[0] / 1.9)) * 100

        info = [
        ("Up", up_count),
        ("Down", down_count),
        ("Person_in", person_count),
        ("Rush ratio", round(rush_ratio, 2))
        ]
